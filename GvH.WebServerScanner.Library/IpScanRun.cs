...
{
    public class IpScanRun
    {
        private readonly ILogger<IpScanRun> logger;

        public IpScanRun(ILogger<IpScanRun> logger)
        {
            this.logger = logger;
        }

        public IpScanRun GetInstance()
        {
            return new IpScanRun(this.logger);
        }

        public IpAddressScanResult PollIpAddress(IpAddressRepresentation ip, IEnumerable<HttpScanParameter> scanParams)
        {
            var res = new IpAddressScanResult() { IpAddress = ip };
            res.PingMs = PingMs(ip);
            if (res.PingMs != null) //Skip if no data was found
            {
        ...
        private IpAddressHttpResult PerformHttpRequest(IpAddressRepresentation ip, HttpScanParameter parameter)
        {
            try
            {
                ...
                logger.LogDebug("Running for:" + composedUrl);
